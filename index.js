...
// ANSWER: Yes, we would change the code to be constant
...
// ANSWER: O(1) It is simplified well. We could condense it to one console.log
...
function squareEveryDigit(number) {
  ...
  for (let i = 0; i < arr.length; i++) {
    const digits = parseInt(arr[i]);
    digits = Math.pow(digits, 2);
    squaredDigits.push(int.toString());
  }
  return squaredDigits.join("");
}

console.log(squareEvertyDigit(9119))
